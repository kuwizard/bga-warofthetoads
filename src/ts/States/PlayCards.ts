...
type CardRole = 'faceUp' | 'faceDown';

export class PlayCards {
    private faceUpCardId: number | null = null;
    private faceDownCardId: number | null = null;
    ...
    private playedCardIds(): number[] {
        return [this.faceUpCardId, this.faceDownCardId].filter((id): id is number => id !== null);
    }

    private async onHandCardClick(cardId: number) {
        if (this.busy) {
            return;
        }
        const role: CardRole = this.faceUpCardId === null ? 'faceUp' : 'faceDown';
    ...
    private async onLaneCardClick(cardId: number) {
        if (this.busy) {
            return;
        }
        const role: CardRole = cardId === this.faceUpCardId ? 'faceUp' : 'faceDown';
    ...
    private setRole(role: CardRole, cardId: number | null) {
        if (role === 'faceUp') {
